jQuery( document ).ready( function( $ ){
	...
	window.applyCanvasBackground = function( tileImg, borderPart ){
		...
		if( borderPart != undefined )
			imgSrc = 'data-border-' + borderPart + '-src';
		...
		window.selected.each( function(  ){
			...
			if( borderPart == undefined ){
				...
				$background = $( '.border-background[data-bg-part="' + borderPart + '"]', this );
			}
	...
	function initThreeJs( envContainer ){
		...
		$( 'canvas.three-bg-container', envContainer ).each( function(  ){
			...
			var geometry = new THREE.PlaneGeometry( $canvas.data( 'width' ), $canvas.data( 'height' ) );
			...
			if( borderIds != undefined ){
				...
				for( var i=0; i < borderIds.length; i++ ){
					...
					if( $border.length > 0 ){
						...
						if( data.xTiles == undefined )
							data.xTiles = 1;
						if( data.yTiles == undefined )
							data.yTiles = 1;
			...
			this.changeTile = function( tileSrc, tileScale ){
				...
				img.onload = function(  ){
					if( tileScale == undefined || tileScale == '' )
						tileScale = 1;
					...
					texture.repeat.set( $canvas.data( 'xrepeat' ) / tileScale, $canvas.data( 'yrepeat' ) / tileScale );
			...
			this.changeBorder = function( borderPart, tileSrc, tileScale ){
				...
				img.onload = function(  ){
					...
					for( var i=0; i < borderPlanes[borderPart].length; i++ ){
						...
						texture.repeat.set( borderPlane.userData.xTiles / tileScale, borderPlane.userData.yTiles / tileScale );
						texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
